...
class InputBox:
    ...
    def handle_key(self, key: int) -> Optional[str]:
        ...
        if key == curses.KEY_BACKSPACE or key == 127 or key == 8:
            ...
            self.text = (
                self.text[: self.cursor_pos] + chr(key) + self.text[self.cursor_pos :]
            )
            self.cursor_pos += 1
...
class MessagesTUI:
    ...
    def _setup_colors(self):
        ...
        try:
            ...
            curses.init_pair(
                self.COLOR_SELECTED, curses.COLOR_BLACK, curses.COLOR_WHITE
            )
    ...
    def _draw_messages(self, autoscroll: bool = False):
